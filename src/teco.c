    ...
    for(idxPos = 0 ; idxPos < k ; ++idxPos){
      ...
      for(cModel = 0 ; cModel < P->nModels ; ++cModel){
        ...
        if(CM->edits != 0){
          ...
          #ifdef RUN_ADDS
          ++n;
          CM->ADDS.idx2 = CM->ADDS.idx;
          CM->ADDS.seq->buf[CM->ADDS.seq->idx] = sym;
          CM->ADDS.idx = GetPModelIdxCorr(CM->ADDS.seq->buf+CM->ADDS.seq->idx-1, 
                         CM, CM->ADDS.idx);
          ComputePModel(CM, pModel[n], CM->ADDS.idx, CM->ADDS.eDen);
          ComputeWeightedFreqs(cModelWeight[n], pModel[n], PT);
          #endif
          #ifdef RUN_DELS
          ++n;
          CM->DELS.seq->buf[CM->DELS.seq->idx] = sym;
          CM->DELS.idx = GetPModelIdxCorr(CM->DELS.seq->buf+CM->DELS.seq->idx-1, 
                         CM, CM->DELS.idx);
          ComputePModel(CM, pModel[n], CM->DELS.idx, CM->DELS.eDen);
          ComputeWeightedFreqs(cModelWeight[n], pModel[n], PT);
          #endif
          }
